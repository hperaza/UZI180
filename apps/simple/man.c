...
/*
 *  Search a whatis file for the next occurence of "title".  Return the
 *  basename of the page to read and the section it is in.  Return 0 on
 *  failure, 1 on success, -1 on EOF or error.
 */
{
    int c, found = 0;
...
/*
 *  Search a windex file for the next occurence of "title".  Return the
 *  basename of the page to read and the section it is in.  Return 0 on
 *  failure, 1 on success, -1 on EOF or error.
 */
{
	...
	if (mid0 == 0) {
	    if (fseek(wf, 0L, SEEK_SET) != 0)
		return -1;
	} else {
	    if (fseek(wf, (long) mid0 - 1L, SEEK_SET) != 0)
		return -1;
 ...
 */
{
    ...
    if (strlen(mandir) + 1 + 6 + 1 > arraysize(whatis))
	return 0;
    ...
    rsp = 0;
    while (!rsp
	   && (rsw = (*searchidx) (wf, title, &wpage, &wsection)) == 1) {
	if (!member(wsection, section))
	    continue;
...
/*
 *  Search the subdirectories of this manual directory for whatis files,
 *  they may have manual pages that override the ones in the major
 *  directory.
 */
{
    char submandir[1024];
...
/* Search the manual path for a manual page describing "title." */
{
	...
	*pd = 0;
	if (trysubmandir(mandir, title, section) && !all)
	    break;
	if (trymandir(mandir, title, section) && !all)
	    break;
    }
}
...
int main(int argc, char **argv)
{
    ...
    if ((pager = getenv("PAGER")) == nil)
	pager = PAGER;
    if ((manpath = getenv("MANPATH")) == nil)
	manpath = MANPATH;
    tty = isatty(1);
