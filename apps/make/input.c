...
struct name *newname(char *name)
{
	...
	if (i > maxsuffarray) {
		...
		sizesuffarray *= 2;
		if ((suffparray =
		     (struct name **) realloc((char *) suffparray,
					      sizesuffarray *
					      sizeof(struct name *))) ==
		    (struct name **) NULL)
		    fatal("No memory for suffarray", (char *) 0, 0);
    ...
    for (rp = (*sp)->n_next, rrp = *sp; rp;
	 rp = rp->n_next, rrp =
	 rrp->n_next) if (strcmp(name, rp->n_name) == 0)
	    return rp;
...
struct cmd *newcmd(char *str, struct cmd *cp)
{
	...
	*rcp = '\0';		/*  Loose newline  */
	...
	(struct cmd *) 0) fatal("No memory for command", (char *) 0, 0);
...
void newline(struct name *np, struct depend *dp, struct cmd *cp, int flag)
{
    ...
    for (rp = np->n_line, rrp = (struct line *) 0; rp;
	 rrp = rp, rp = rp->l_next) if (rp->l_cmd)
	    hascmds = TRUE;
