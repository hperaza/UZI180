...
char *crypt(char *key, char *salt)
{
    ...
    *rkey = *salt;
    ...
    /*
     *  Now we need to unpack the bits and map it to "A-Za-z0-9./" for
     *  printing in /etc/passwd
     */
	...
	if (rkey[i] >= 0 && rkey[i] < 12)
	    rkey[i] += 46;
	else if (rkey[i] > 11 && rkey[i] < 38)
	    rkey[i] += 53;
	else if (rkey[i] > 37 && rkey[i] < 64)
	    rkey[i] += 59;
	else
	    return NULL;
    }
