...
{
#ifdef UZI
  ioctl(0, TIOCGETP, (char *) &_orig_tty);
  ioctl(0, TIOCGETP, (char *) &_tty);
  _cursvar.echoit = (_tty.t_flags & ECHO) != 0;
  _cursvar.rawmode = (_tty.t_flags & RAW) != 0;
  _cursvar.cbrkmode = (_tty.t_flags & CBREAK) != 0;
  NONL = (_tty.t_flags & CRMOD) == 0;
#else
